...
set_rdhs_config(email = "kendavidn@gmail.com" ,
                project = "The effect of birth spacing on maternal and child health outcomes in SSA", password_prompt = T
                # ,
...
# reshape to long
sib_long. <- mortDHS::mortDHS_reshape(data = bound_dhs, sib_cols = c(1,2,4,8)) 
# add in interview date variable. NEEDED FOR MORTALITY CALC
sib_long <- mortDHS::mortDHS_merge(sib_long., bound_dhs, respondent_vars = c(individual_cols, "surveyid") ) 
...
sib_long$obs_time <- mortDHS::mortDHS_obs_time(sib_long)
